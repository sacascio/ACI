...
def add_quad_zero(subnet,ip,cookie,tenant,extepg,l3out):
	...
	if response.status_code == 200:
                print ("OK: Subnet 0.0.0.0/0 added to external EPG %s " % (extepg))
	else:
		print (response.json())

def delete_subnet(subnet,ip,cookie,tenant,extepg,l3out):
	...
	if response.status_code == 200:
                print ("OK: Subnet %s removed from external EPG %s " % (subnet,extepg))
	else:
		print (response.json())
	...
	if response.status_code == 200:
		...
		if len(subnets) == 0:
			...
			for s in sorted_subnets:
				delete_subnet(s,f_ip,cookie,tenant,extepg,l3out)

		add_quad_zero(s,f_ip,cookie,tenant,extepg,l3out)
